...
#mydata = genfromtxt('/home/username/Hybrid_Deep_Reinforcement_Learning_RoughTerrain/OmniIsaacGymEnvs/omniisaacgymenvs/Waypoints/Silverstone_centerline.csv', delimiter = ',')
crosstrack_error = []
yaw_error = []
vel = []
xpos = []
...
class HunterTaskE2E(RLTask):
    def __init__(
        ...
        self._hunterse_positions = torch.tensor([0.0, 0.0, 0.2])
    ...
    def set_up_scene(self, scene) -> None:
        ...
        self.delta_in = torch.zeros(self._num_envs, 1)
        self.delta_out = torch.zeros(self._num_envs, 1)
    ...
    def get_observations(self):
        ...
        for i in range(self._num_envs):
            ...
            self.obs_buf[i, 2] = yaw_world[i]# Yaw
            self.obs_buf[i, 3] = base_lin_vel[i]
            self.obs_buf[i, 4] = crosstrack_error
            self.obs_buf[i, 5] = yaw_error
        #self.obs_buf[:,6] = roll_local
        observations = {
            self._hunterse.name: {
    ...
    def disturb_robot(self):
        ...
        self.root_velocities[:,5] = torch.randint(-3,3,(self._num_envs,1), device=self._device
                                                     )
    ...
    def reset_idx(self, env_ids=None):
        ...
        self._hunterse.set_joint_velocities(dof_vel, indices)
    ...
    def pre_physics_step(self, actions) -> None:
        ...
        self.position[:, self._front_steer_joint_idx] = torch.clamp(0.524*actions[:, 0], min=-0.524, max=0.524)
                   
        actions[:, 1] = torch.clamp(21.82*actions[:, 1], min=5.0, max=21.82)
        
        self.vels[:, self._rwl_dof_idx] = actions[:,1]# Apply torques b/w the given range                                               
    ...
    def calculate_metrics(self) -> None:
        ...
        _,self.pitch_local,_ = get_euler_xyz(chassis_rotation_local)
        self.pitch_local = torch.where(self.pitch_local>=torch.pi, self.pitch_local - 2*torch.pi, self.pitch_local)
       ...
       ##
        ...
        pitch_numpy = self.pitch_local.cpu().numpy()
        ...
        reward = torch.clip(reward, min=0.0, max=None)
       

        self.rew_buf[:] = reward

    def is_done(self) -> None:
        hunter_xvel = self.obs_buf[:,4]
        
        
        resets = torch.where(torch.abs(self.obs_buf[:,4]) >= 10.0, 1, 0)
